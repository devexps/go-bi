(function (window, undefined) {
    "use strict";
    var LIBVERSION = "1.0.0", EMPTY = "", UNKNOWN = "?", FUNC_TYPE = "function", UNDEF_TYPE = "undefined",
...
EventReport.prototype.getTimeStamp = function () {
    var time = Date.parse(new Date()).toString(); // Get the timestamp in milliseconds, accurate to milliseconds
    time = time.substr(0, 10); //accurate to the second
    return time;
}
...
EventReport.prototype.trackUserData = function () {
    ...
    this.ajax(this.getReportApi("User Attributes", this.reportUser), userProperties, (res) => {
        if (this.debug === 1 || this.debug === 2) {
            console.log("res", res);
        }
    }, (err) => {
...
EventReport.prototype.getDeviceModel = function () {
    var canvas = document.createElement('canvas'), gl = canvas.getContext('experimental-webgl'),
